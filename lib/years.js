...
(function (root) {
  ...
  function timeline(beers) {
    ...
    for (const b of valid) {
      ...
      byMonth.set(b.first.slice(0, 7), { unique, breweries: seenBreweries.size, styles: seenStyles.size });
    }
    ...
    const lastKey = [...byMonth.keys()].at(-1);
    let cur = { unique: 0, breweries: 0, styles: 0 };
  ...
  const api = { buildYears, compareYear, timeline, monthBreakdown, statsFor, topCounts, firstSeen };
  root.DFU = root.DFU || {};
  root.DFU.years = api;
