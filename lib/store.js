...
(function (root) {
  ...
  async function saveHistory(user, { beers, complete }) {
    const value = { beers, syncedAt: Date.now(), complete: !!complete, count: beers.length };
  ...
  async function loadCountries(user) {
    ...
    const r = await storage().get(k);
    return r[k] ?? { byBeer: {}, at: null, complete: false, count: 0 };
  }

  async function saveCountries(user, { byBeer, complete }) {
    const value = { byBeer, at: Date.now(), complete: !!complete, count: Object.keys(byBeer).length };
    await storage().set({ [countriesKey(user)]: value });
    return value;
  }

  root.DFU.store = { ...pure, load, save, loadHistory, saveHistory, loadCountries, saveCountries, getSettings, setSettings, lastUser, clearAll, DEFAULT_SETTINGS };
