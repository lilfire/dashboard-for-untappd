...
(function (root) {
  ...
  async function syncCountries(user, { countries = [], onProgress, signal } = {}) {
    const byBeer = {};
    let tabId = null;
    let via = 'direct';
    let pages = 0;
    ...
    try {
      for (const [index, country] of countries.entries()) {
        for (let off = 0; off < MAX_PAGES * PAGE_SIZE; off += PAGE_SIZE) {
          ...
          if (beers.length < PAGE_SIZE) break;
          await sleep(DELAY_MS, signal);
        }
        await sleep(DELAY_MS, signal);
      }
    ...
    if (tabId != null) closeTabSession(tabId);
    return { byBeer, pages, via, stopped, complete: stopped === 'end' };
  }

  const api = { sync, syncCountries, merge, pageIsKnown, PAGE_SIZE, DELAY_MS };
  root.DFU = root.DFU || {};
