...
(function (root) {
  ...
  async function sync(user, { known = [], full = false, expected = null, onProgress, signal } = {}) {
    ...
    const getPage = async off => {
      const link = url(user, off);
      if (tabId != null) return fetchInTab(tabId, link);
      try {
        const beers = await request(link, signal);
    ...
    return { beers, pages, fetched: collected.size, stopped, via, complete: full || stopped === 'known' || pages < MAX_PAGES };
  }
